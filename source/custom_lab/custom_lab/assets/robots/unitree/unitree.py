...
from custom_lab.assets.robots.unitree import unitree_actuators
import os


THIS_DIR = os.path.dirname(os.path.abspath(__file__))
UNITREE_MODEL_DIR = os.path.join(THIS_DIR).__str__()  # Replace with the actual path to your unitree_model directory
UNITREE_ROS_DIR = os.path.join(THIS_DIR).__str__()  # Replace with the actual path to your unitree_ros package

g1_usd_path_cache = f"{UNITREE_MODEL_DIR}/../../usd/unitree/g1/g1_29dof_rev_1_0.usd"  # <- 替换为实际 usd 文件路径
...
class UnitreeUrdfFileCfg(sim_utils.UrdfFileCfg):
    ...
    joint_drive = sim_utils.UrdfConverterCfg.JointDriveCfg(gains=sim_utils.UrdfConverterCfg.JointDriveCfg.PDGainsCfg(stiffness=0, damping=0))
    articulation_props = sim_utils.ArticulationRootPropertiesCfg(
        enabled_self_collisions=True,
        solver_position_iteration_count=8,
        solver_velocity_iteration_count=4,
...
from custom_lab.env.assets.articulation_cfg import ArticulationCfgCM


@configclass
class UnitreeArticulationCfgCM(ArticulationCfgCM):
    """Configuration for Unitree articulations."""
...
UNITREE_G1_29DOF_CFG = UnitreeArticulationCfgCM(
    # spawn=UnitreeUrdfFileCfg(
    #     asset_path=f"{UNITREE_ROS_DIR}/robots/g1_description/g1_29dof_rev_1_0.urdf",
    # ),
    spawn=UnitreeUsdFileCfg(
        usd_path=g1_usd_path_cache,
        activate_contact_sensors=True,
    ),
    init_state=ArticulationCfgCM.InitialStateCfg(
        pos=(0.0, 0.0, 0.8),
        joint_pos={
...
UNITREE_G1_29DOF_MIMIC_CFG = UnitreeArticulationCfgCM(
    # spawn=UnitreeUrdfFileCfg(
    #     asset_path=f"{UNITREE_ROS_DIR}/robots/g1_description/g1_29dof_rev_1_0.urdf",
    # ),
    spawn=UnitreeUsdFileCfg(
