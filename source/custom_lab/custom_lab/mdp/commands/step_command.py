...
class StepCommand(CommandTerm):
    ...
    def _debug_vis_callback(self, event):
        ...
        right_step_command_loc = torch.hstack(
            (self._env.step_commands[:, 0, 0:1], self._env.step_commands[:, 0, 1:2], torch.zeros(self.num_envs, 1, device=self.device))
        )
        left_step_command_loc = torch.hstack(
            (self._env.step_commands[:, 1, 0:1], self._env.step_commands[:, 1, 1:2], torch.zeros(self.num_envs, 1, device=self.device))
        )

        right_step_command_quat = math_utils.quat_from_euler_xyz(
            torch.zeros(self.num_envs, device=self.device), torch.zeros(self.num_envs, device=self.device), self._env.step_commands[:, 0, 2]
        )
        left_step_command_quat = math_utils.quat_from_euler_xyz(
            torch.zeros(self.num_envs, device=self.device), torch.zeros(self.num_envs, device=self.device), self._env.step_commands[:, 1, 2]
        )
        ...
        right_current_step_loc = torch.hstack(
            (self._env.current_step[:, 0, 0:1], self._env.current_step[:, 0, 1:2], torch.zeros(self.num_envs, 1, device=self.device))
        )
        left_current_step_loc = torch.hstack(
            (self._env.current_step[:, 1, 0:1], self._env.current_step[:, 1, 1:2], torch.zeros(self.num_envs, 1, device=self.device))
        )

        right_current_step_quat = math_utils.quat_from_euler_xyz(
            torch.zeros(self.num_envs, device=self.device), torch.zeros(self.num_envs, device=self.device), self._env.current_step[:, 0, 2]
        )
        left_current_step_quat = math_utils.quat_from_euler_xyz(
            torch.zeros(self.num_envs, device=self.device), torch.zeros(self.num_envs, device=self.device), self._env.current_step[:, 1, 2]
        )
