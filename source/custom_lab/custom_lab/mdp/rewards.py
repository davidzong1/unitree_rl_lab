...
import torch
from custom_lab.utils.math import *
...
def joint_mirror(env: ManagerBasedRLEnv, asset_cfg: SceneEntityCfg, joint_name_pairs: list[list[str]]) -> torch.Tensor:
    # extract the used quantities (to enable type-hinting)
    asset: Articulation = env.scene[asset_cfg.name]
    if not hasattr(env, "joint_mirror_joints_cache") or env.joint_mirror_joints_cache is None:
        # Cache joint positions for all pairs
        env.joint_mirror_joints_cache = [[asset.find_joints(joint_name) for joint_name in joint_pair] for joint_pair in joint_name_pairs]
    ...
    reward *= 1 / len(joint_name_pairs) if len(joint_name_pairs) > 0 else 0
    return reward


def track_lin_vel_xy_yaw_frame_exp_reward(
    env: ManagerBasedRLEnv,
    std: float,
    asset_cfg: SceneEntityCfg = SceneEntityCfg("robot"),
    command_name: str = "base_velocity",
    ...
    return _ne_sigmodial(lin_vel_error, scale=5.0)
...
def tracking_CAM_reward(env: ManagerBasedRLEnv, asset_cfg: SceneEntityCfg, command_name: str):
    ...
    return ne_sigmodial(error, scale=3.0)
...
def CAM_compensation_reward(env: ManagerBasedRLEnv):
    ...
    return ne_sigmodial(env.CM_leg[:, -1] + env.CM_arm[:, -1], 5.0)
