...
def check_work_env(str_kernel: str) -> str:
    """检查work_env变量是否只出现一次，若是则在前面添加//加以注释"""
    work_env_count = str_kernel.count('work_env')
    if work_env_count == 1:
        # 只声明了一次但未使用，注释掉声明行
        lines = str_kernel.split('\n')
        for i, line in enumerate(lines):
            if 'float work_env[' in line and not line.strip().startswith('//'):
                lines[i] = '    // ' + line.strip()
                break
        return '\n'.join(lines)
    return str_kernel

def generateCMakeLists(casadi_fns,cuda_name:list[str]=[]):
    file_name:list[str]=[]
    if cuda_name is None:
        file_name = [f.name() for f in casadi_fns]
    else:
        file_name = cuda_name
    ...
    for f in casadi_fns:
        print(file_name[cnt])
        fn_source_name = file_name[cnt].upper() + "_SOURCE"
        fn_filepath = f"codegen/{file_name[cnt]}.cu"
        str_sources += f"set({fn_source_name} {fn_filepath})\n"
        str_libraries += f"add_library({file_name[cnt]} SHARED ${{{fn_source_name}}})\n"
        str_libraries += f"target_link_libraries({file_name[cnt]})\n"
        cnt+=1
...
def generateCUDACodeDouble(f, cuda_name=None, filepath=None, benchmarking=True, debug_mode=True):
    print("Generating CUDA code for CasADi function: ", f.name())
    if filepath is None:
        if cuda_name is None:
            codegen_filepath = os.path.join(CUSADI_ROOT_DIR, "codegen", f"{f.name()}.cu")
        else:
            codegen_filepath = os.path.join(CUSADI_ROOT_DIR, "codegen", f"{cuda_name}.cu")
    else:
    ...
    str_kernel += "\n    }"         # End of if statement
    str_kernel += "\n}\n"           # End of kernel
    # str_kernel = check_work_env(str_kernel)
    ...
    if cuda_name is None:
        ...
        print("CUDA codegen complete for CasADi function: ", cuda_name)

def generateCUDACodeFloat(f, cuda_name=None, filepath=None, benchmarking=True, debug_mode=True):
    if filepath is None:
        if cuda_name is None:
            print("Generating CUDA code for CasADi function: ", f.name())
            codegen_filepath = os.path.join(CUSADI_ROOT_DIR, "codegen", f"{f.name()}.cu")
        else:
            print("Generating CUDA code for CasADi function: ", cuda_name)
            codegen_filepath = os.path.join(CUSADI_ROOT_DIR, "codegen", f"{cuda_name}.cu")
    else:
        codegen_filepath = filepath
    print("Codegen filepath: ", codegen_filepath)
    codegen_file = open(codegen_filepath, "w+")
    codegen_strings = {}
...
def generateCUDACodeOrig(f,cuda_name=None, filepath=None, benchmarking=True, debug_mode=True):
    if filepath is None:
        if cuda_name is None:
            print("Generating CUDA code for CasADi function: ", f.name())
            codegen_filepath = os.path.join(CUSADI_ROOT_DIR, "codegen", f"{f.name()}.cu")
